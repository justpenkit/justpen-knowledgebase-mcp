...
@pytest.mark.parametrize(
    ("settings", "field"),
    [({"PORT": "SECRET-MARKER"}, "port"), ({"TRANSPORT": "http", "HOST": "SECRET-MARKER"}, "host")],
)
def test_environment_validation_diagnostics_never_include_input(settings, field):
    env = {PREFIX + "WORKSPACE_DIR": "/workspace", **{PREFIX + key: value for key, value in settings.items()}}
